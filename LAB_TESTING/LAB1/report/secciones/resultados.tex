...
La guía de práctica nombra originalmente una IMU para esta etapa; el sensor realmente disponible es un BME280 (presión, temperatura, humedad), sin canales de aceleración ni de velocidad angular (Sección de Metodología). La comparación ``sensor quieto'' vs. ``sensor en movimiento'' de la guía se adapta en consecuencia a \textbf{condiciones base} (ambiente estable, sensor sin perturbar) vs. \textbf{condiciones perturbadas} (por ejemplo, acercar una fuente de calor o exhalar cerca del sensor para elevar temperatura/humedad localmente, o cubrirlo parcialmente para alterar la presión leída) — la magnitud física perturbada no es movimiento, sino cambios ambientales deliberados.

\begin{table}[!t]
\centering
\caption{Estadística BME280 — condiciones base}
\label{tab:bme280-base}
\small
\begin{tabular}{@{}l r r r r@{}}
\toprule
Canal & Media & Desv. est. & Nivel DC & Ancho de banda \\
\midrule
Temperatura (°C) & pendiente & pendiente & pendiente & pendiente \\
Presión (hPa) & pendiente & pendiente & pendiente & pendiente \\
Humedad (\%RH) & pendiente & pendiente & pendiente & pendiente \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[!t]
\centering
\caption{Estadística BME280 — condiciones perturbadas}
\label{tab:bme280-perturbado}
\small
\begin{tabular}{@{}l r r r r@{}}
\toprule
Canal & Media & Desv. est. & Nivel DC & Ancho de banda \\
...
Las Tablas~\ref{tab:bme280-base} y \ref{tab:bme280-perturbado} se completan ejecutando \texttt{matlab/analyze\_bme280.m} sobre \texttt{data/bme280\_baseline/*.csv} y \texttt{data/bme280\_perturbed/*.csv} respectivamente, generados con \texttt{pc\_logger/serial\_logger.py --mode bme280} a $100$ Hz, no $200$ Hz — límite propio del sensor, ver Sección de Metodología.
