"""Audio loading for the cat-audio filtering case (C.3).

Mirrors `WORK_THREE/animal_analyzer/core.py::load_audio` and
`WORK_FOUR/conv_lab/audio.py`: `librosa` is imported lazily so the rest of
this package and its tests stay importable without it installed.
"""
